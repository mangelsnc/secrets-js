...
if (commandHandler.hasOwnProperty(subcommand)) {
    ...
    console.log("Usage: secrets [init|generate-keypair|list|set|get|dump]\n")
}
...
function handleInit() {
    if (!fs.existsSync(process.env.PWD + '/secrets.conf.json')) {
        fs.copyFileSync('config/default.json', process.env.PWD + '/secrets.conf.json');
    }
...
function loadConfig() {
    ...
    const userConfig = JSON.parse(fs.readFileSync(process.env.PWD + '/secrets.conf.json'));
